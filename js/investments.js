...
// Renderizar lista de investimentos (formato tabela)
function renderInvestments() {
  ...
  const rows = allInvestments.map(inv => {
    ...
    const typLabel = inv.type === 'stock' ? 'Ação' : inv.type === 'fii' ? 'FII' : 'FII';
    const typBadge = inv.type === 'stock' ? 'badge-stock' : 'badge-fii';
    totalInvested += invested;
    totalCurrent += currentTotal;

    return '<tr class="inv-row">'
      + '<td><span class="inv-code">' + inv.ticker + '</span></td>'
      + '<td><div class="inv-name-cell">' + (inv.name || inv.ticker) + '</div><span class="badge ' + typBadge + '">' + typLabel + '</span></td>'
      + '<td style="text-align:center">' + inv.quantity + '</td>'
      + '<td style="text-align:right;font-family:var(--font-mono)">' + formatCurrency(inv.avg_price) + '</td>'
      + '<td style="text-align:right;font-family:var(--font-mono);font-weight:700">' + formatCurrency(currentPrice) + '</td>'
      + '<td style="text-align:right;font-family:var(--font-mono)">' + formatCurrency(currentTotal) + '</td>'
      + '<td style="text-align:right" class="' + gainCls + '">' + gainSign + formatCurrency(gain) + '<div style="font-size:10px">' + formatPercent(gainPct) + '</div></td>'
  ...
  container.innerHTML = '<div class="inv-table-wrap"><table class="inv-table">'
    + '<thead><tr>'
    + '<th>Código</th><th>Nome</th><th style="text-align:center">Qtd</th>'
    + '<th style="text-align:right">Preço médio</th><th style="text-align:right">Preço atual</th>'
    + '<th style="text-align:right">Total atual</th><th style="text-align:right">Resultado</th>'
    + '<th></th>'
    + '</tr></thead>'
    + '<tbody>' + rows + '</tbody>'
    + '</table></div>';
  ...
  // Atualizar mini-lista na visão geral também
  renderInvestmentsOverview();
}

// Mini lista de investimentos para visão geral
function renderInvestmentsOverview() {
  const el = document.getElementById('overview-inv-list');
  if (!el) return;
  if (!allInvestments.length) {
    el.innerHTML = '<div class="empty-state" style="padding:16px">Nenhum investimento cadastrado</div>';
    return;
  }
  const rows = allInvestments.slice(0, 6).map(inv => {
    const currentPrice = inv.current_price || inv.avg_price;
    const gain = (currentPrice - inv.avg_price) / inv.avg_price * 100;
    const cls = gain >= 0 ? 'positive' : 'negative';
    return '<div class="list-item" style="padding:8px 10px">'
      + '<div style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--accent);min-width:60px">' + inv.ticker + '</div>'
      + '<div class="item-info"><div style="font-size:12px;font-weight:600">' + (inv.name || inv.ticker) + '</div>'
      + '<div style="font-size:10px;color:var(--text-muted)">' + inv.quantity + ' cotas</div></div>'
      + '<div style="text-align:right">'
      + '<div style="font-family:var(--font-mono);font-size:12px;font-weight:700">' + formatCurrency(currentPrice) + '</div>'
      + '<div class="' + cls + '" style="font-size:10px">' + formatPercent(gain) + '</div>'
      + '</div></div>';
  }).join('');
  el.innerHTML = rows + (allInvestments.length > 6 ? '<div style="text-align:center;padding:8px;font-size:11px;color:var(--text-muted)">+ ' + (allInvestments.length - 6) + ' outros</div>' : '');
}
