...
function renderInvestments() {
  ...
  container.innerHTML = allInvestments.map(inv => {
    const cp = cachedPrices[inv.ticker];
    ...
    return `
      <div class="investment-item card-hover" data-id="${inv.id}">
        <div class="inv-ticker">${inv.ticker}</div>
        <div class="inv-info">
          <div class="inv-name">${inv.name || inv.ticker}</div>
          <div class="inv-type badge badge-${inv.type}">${inv.type === 'stock' ? 'Ação' : inv.type === 'fii' ? 'FII' : 'Outro'}</div>
        </div>
        <div class="inv-qty">${inv.quantity} cotas</div>
        <div class="inv-price">
          <div>${formatCurrency(currentPrice)}</div>
          <div class="${gainPct >= 0 ? 'positive' : 'negative'}" style="font-size:11px;">
            ${formatPercent(gainPct)}
          </div>
        </div>
        <div class="inv-total">
          <div>${formatCurrency(currentTotal)}</div>
          <div class="${gain >= 0 ? 'positive' : 'negative'}" style="font-size:11px;">
  ...
  if (el) {
    el.innerHTML = `
      <span>Investido: <strong>${formatCurrency(totalInvested)}</strong></span>
      <span>Atual: <strong>${formatCurrency(totalCurrent)}</strong></span>
      <span class="${totalGain >= 0 ? 'positive' : 'negative'}">
        ${totalGain >= 0 ? '+' : ''}${formatCurrency(totalGain)} (${formatPercent(totalGain / totalInvested * 100)})
      </span>
    `;
  }
}
