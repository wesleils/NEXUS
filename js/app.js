...
function renderDashboard() {
  ...
  renderDashInvestments();
  renderDashCalendar();
  renderCalendarOverview();
}

// Data e hora na quickbar
function renderDashQuickbar() {
  const el = document.getElementById('dq-datetime');
  if (!el) return;
  const now = new Date();
...
// Gastos e receitas recentes no dashboard
function renderDashTransactions() {
  const expEl = document.getElementById('dash-expenses-list');
  const incEl = document.getElementById('dash-income-list');

  const expenses = state.transactions.filter(t => t.type === 'expense').slice(0,5);
  const incomes  = state.transactions.filter(t => t.type === 'income').slice(0,5);

  function buildList(txs, container) {
    if (!container) return;
    if (!txs.length) { container.innerHTML = '<div class="dash-inv-empty">Nenhum registro</div>'; return; }
    container.innerHTML = txs.map(tx => {
      ...
      return '<div class="dash-mini-item">'
        + '<div class="dash-mini-icon" style="background:' + color + '22;color:' + color + '">' + icon + '</div>'
        + '<div style="flex:1;min-width:0">'
        + '<div class="dash-mini-name" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + tx.description + '</div>'
        + '<div class="dash-mini-sub">' + formatDate(tx.date) + '</div>'
        + '</div>'
        + '<div class="dash-mini-val ' + cls + '">' + sign + formatCurrency(tx.amount) + '</div>'
        + '</div>';
    }).join('');
  }

  buildList(expenses, expEl);
  buildList(incomes,  incEl);
}

// Ações e FIIs no dashboard
function renderDashInvestments() {
  const stocksEl = document.getElementById('dash-stocks-list');
  const fiisEl   = document.getElementById('dash-fiis-list');

  const stocks = allInvestments.filter(i => i.type === 'stock');
  const fiis   = allInvestments.filter(i => i.type === 'fii');

  function buildInvList(invs, container) {
    if (!container) return;
    if (!invs.length) {
      container.innerHTML = '<div class="dash-inv-empty">Nenhum cadastrado</div>';
      return;
    }
    container.innerHTML = invs.slice(0, 6).map(inv => {
      const price = inv.current_price || inv.avg_price;
      const change = inv.avg_price ? ((price - inv.avg_price) / inv.avg_price * 100) : 0;
      const cls = change >= 0 ? 'positive' : 'negative';
      const sign = change >= 0 ? '+' : '';
      return '<div class="dash-inv-item" onclick="goTab(\'investments\')">'
        + '<div class="dash-inv-code">' + inv.ticker + '</div>'
        + '<div class="dash-inv-name">' + (inv.name || inv.ticker) + '</div>'
        + '<div style="text-align:right">'
        + '<div class="dash-inv-price">' + formatCurrency(price) + '</div>'
        + '<div class="dash-inv-var ' + cls + '">' + sign + change.toFixed(2) + '%</div>'
        + '</div>'
        + '</div>';
    }).join('');
  }

  buildInvList(stocks, stocksEl);
  buildInvList(fiis,   fiisEl);
}

// Calendário compacto no dashboard
function renderDashCalendar() {
  ...
  const monthName = now.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
  const calTitle = document.getElementById('dash-cal-month');
  if (calTitle) calTitle.textContent = monthName.charAt(0).toUpperCase() + monthName.slice(1);

  // Grid de dias da semana
  let html = '<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;margin-bottom:2px">';
  ['D','S','T','Q','Q','S','S'].forEach(d => {
    html += '<div style="text-align:center;font-size:8px;font-weight:700;color:var(--text-muted);padding:2px">' + d + '</div>';
  });
  html += '</div>';
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    const bills = state.calendarBills.filter(b => b.due_day === d);
    const debts = state.debts.filter(db => db.due_day === d && db.is_active);
    const isToday = d === today;
    const hasBill = bills.length > 0 || debts.length > 0;
    const firstColor = bills[0]?.color || debts[0]?.color || 'var(--warning)';

    const bg = isToday ? 'background:var(--accent)22;border:1px solid var(--accent);color:var(--accent);font-weight:800'
      : hasBill ? 'background:var(--surface2);border:1px solid ' + firstColor + '60'
      : 'background:var(--surface2);border:1px solid transparent';

    html += '<div style="height:24px;display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:4px;font-size:9px;font-weight:600;cursor:pointer;gap:1px;' + bg + '">'
      + d
      + (hasBill ? '<div style="width:3px;height:3px;border-radius:50%;background:' + firstColor + '"></div>' : '')
      + '</div>';
  }
  html += '</div>';
  container.innerHTML = html;
