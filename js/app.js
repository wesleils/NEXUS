...
async function initApp() {
  try {
    // Aplicar tema padrão e mostrar login imediatamente
    applyTheme('nexus-dark');
    showLoader(false);
    showAuthScreen();

    // Verificar sessão ativa em background
...
function renderSummaryCards() {
  ...
  if (overviewList && state.transactions.length) {
    ...
    overviewList.innerHTML = top.map(tx => \`
      <div class="list-item">
        <div class="item-icon" style="background:\${tx.categories?.color || 'var(--accent)'}22;color:\${tx.categories?.color || 'var(--accent)'}">
          \${tx.categories?.icon || (tx.type === 'income' ? '💰' : '💸')}
        </div>
        <div class="item-info">
          <div class="item-name">\${tx.description}</div>
          <div class="item-sub">\${tx.categories?.name || 'Sem categoria'} · \${formatDate(tx.date)}</div>
        </div>
        <div class="item-value \${tx.type === 'income' ? 'positive' : 'negative'}">
