...
function renderDashCalendar() {
  ...
  ['D','S','T','Q','Q','S','S'].forEach(d => { html += '<div class="dash-cal-wd">' + d + '</div>'; });
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const dots = [...bills, ...debts].slice(0,2).map(b => 
      '<div class="dash-cal-dot" style="background:' + (b.color || 'var(--warning)') + '"></div>'
    ).join('');

    html += '<div class="dash-cal-day' + (isToday ? ' today' : '') + (hasBill ? ' has-bill' : '') + '">'
      + d + dots + '</div>';
  }
  html += '</div>';
  container.innerHTML = html;
