...
// ── GRÁFICO 2: Gastos por categoria — donut com glow ─────────
async function renderCategoryChart(transactions) {
  ...
  const sorted = Object.entries(byCat).sort((a,b) => b[1].total - a[1].total).slice(0,7);
  ...
  const palette = [
    c.accent, c.info, c.positive, c.warning, c.negative, c.accent2,
  ...
  charts['categories'] = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: sorted.map(([,v],i) => alpha(v.color || palette[i % palette.length], 0.85)),
        borderColor: sorted.map(([,v],i) => v.color || palette[i % palette.length]),
        borderWidth: 2,
        hoverOffset: 10,
        hoverBorderWidth: 3,
      }]
